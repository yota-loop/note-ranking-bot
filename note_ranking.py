...
環境変数（automation/.env）:
  WP_SITE_URL     ... 例 https://www.good-daily-life.com/kouiunolab
  WP_USERNAME     ... WordPressログインユーザー名
  WP_APP_PASSWORD ... WordPress Application Password（プロフィール画面で発行）
"""
...
def push_to_wordpress(body_content: str) -> str | None:
    """WordPress固定ページとして投稿・更新する（REST API + Application Password）。
    成功時はページURLを返す。
    ※ XML-RPC経由だとXserver側で理由不明の403 Forbiddenが発生したため、REST APIに切り替えている。"""
    _load_env()
    site_url = os.environ.get("WP_SITE_URL")
    username = os.environ.get("WP_USERNAME")
    app_password = os.environ.get("WP_APP_PASSWORD")
    if not (site_url and username and app_password):
        print("[note_ranking] WordPress未設定（.envにWP_SITE_URL/WP_USERNAME/WP_APP_PASSWORDが必要）。投稿をスキップします")
        return None
    ...
    path = f"pages/{existing_id}" if existing_id else "pages"
    ...
    try:
        ...
        if not existing_id:
            WP_PAGE_ID_FILE.write_text(str(data["id"]), encoding="utf-8")
